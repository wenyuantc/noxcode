...
- 重试：`model_retry_*` 设置控制指数退避（默认 6 次、1 s 起、上限 30 s、倍数 2、带抖动），服务端 `Retry-After` 优先但不超过上限。不另起一套恢复重试。每次尝试写入 `native_api_call_logs.attempt`。首包等待、流空闲、请求总期限、网络中断和缺少终止事件的半截流可重试；用户取消、协议错误、上下文上限和 4xx 认证/参数错误不重试。重试前清空已流式显示的半截内容，前端通过 `[重试]` 行看到原因、等待时间和 `第 n/6 次`。
