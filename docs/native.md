...
- `Computer`：本机桌面控制（实现见 [`tools/desktop.rs`](../src-tauri/src/native/tools/desktop.rs)）。单一工具、单一权限能力 `computer`，动作包括 `screenshot` / `click` / `double_click` / `move` / `drag` / `scroll` / `type` / `keypress` / `wait`。坐标相对**本次截图**左上角像素；若缩放到长边约 1280，点击前会把模型坐标乘回物理 / 显示器逻辑像素。写动作成功后自动再截一张图，经 `ToolOutput.images` 交给模型，文本带 `display`、`width`、`height`、`scale_x` / `scale_y`。契约为 `SideEffectScope::System`、`RiskLevel::High`、`needs_approval: true`、`destructive: true`、`concurrent_safe: false`、`allowed_in_plan_mode: false`，超时约 15 秒。设置项 `computer_control_enabled` 默认关，写在 `native-settings.json`，无需新迁移。仅当开关打开、本地会话、非只读、非计划模式、且不是子 Agent 时才广告该工具；关闭或 SSH 误调用返回「电脑控制未开启 / SSH 工作区不可用」。`classify_native_tool_risk` 对 `Computer` 返回 `kind = computer`，走现有权限弹窗；「始终允许」按工具名写入 `capability=computer`。`yolo` 仍尊重显式 deny，总开关关闭时 yolo 也不能用。macOS 需要屏幕录制与辅助功能；Windows 按显示器 DPI 映射坐标，UAC 安全桌面会明确失败；Linux 以 X11 为完整路径，Wayland 截屏尽量走 portal，注入失败则提示授权 Remote Desktop portal 或改用 X11。全局串行锁保证同一时刻只执行一个桌面动作。设置页 `/settings/computer` 提供总开关、系统权限状态和「打开系统设置」；命令经 [`backend.ts`](../src/lib/backend.ts) 的 `getComputerPermissionStatus` / `openComputerPrivacySettings`，前端不直连 OS API。
