...
- 目标（[`goals.rs`](../src-tauri/src/native/goals.rs)）：`Goal(action=set|update|complete|clear, title, checklist, criteria, note)` 维护会话目标。`criteria` 是版本化验收条件，`kind` 为 `test`、`artifact` 或 `subjective`。`complete` 只发起核验，不直接把清单勾完。测试条件只认已提交的 Bash 结果，产物条件由后端读取文件，主观条件把产物交给只读的轻量模型复核。失败、超时、取消、没有证据都保持未完成。可纠正失败最多自动继续 3 次，次数持久化，用完后暂停并展示未满足项。条件、分支或产物变化后旧证据失效。`[GOAL]` 行和 `list_native_goal_verifications` 能看到核验状态、证据、失败原因和剩余次数。
