...
事件流底部除后台子 Agent 面板外，还有 Bash / Monitor 进程表（`list_native_background_processes` / `stop_native_background_process`）。设置页「工具运行时」可开关 LSP 与本地 Bash 沙箱。首页 Composer 工作区/分支旁是「当前工作区 | 隔离工作树」两段切换，新会话经 `isolate_worktree` 启动。使用了隔离的会话可在标题栏合并工作树；本轮结束（`waiting_input`）和 `native-exit` 都会弹出同一对话框。弹窗按 `sessionId` 重挂并在打开时清空提交说明 / 分支名，避免换会话仍带着上一次的表单。弹窗里的提交说明框与 Git 侧栏相同：可手写，开启 AI 提交说明后可用 Sparkles 按隔离 worktree 的未提交改动生成。Git 侧栏对隔离会话展示该 worktree 的变更，文件预览允许 worktree 路径。冲突时「AI 自动解决」把说明发进当前会话（关闭弹窗，不在弹窗里干等 one-shot）；回合结束后若文件已无冲突标记则完成合并，否则再弹出冲突框，并始终把会话切回隔离 worktree，避免后续对话落到主仓 git 路径。也可打开 Git 面板手动处理。`NativeSessionRuntime` 可带 `worktree_path` 与 `sandbox_active`。
...
左导航三组：基础设置（general / appearance / channels / ai / ssh）、Agent 能力（runtime / permissions / memory / automations / subagents / mcp / skills / hooks / worktrees）、数据与统计（usage / database / about）。`/settings/worktrees` 可配置工作树根目录、创建前拉取、自动清理数量，并列出 / 删除托管 worktree。`/settings/ai` 提供两张卡：Git 提交信息与会话标题，各有开关和独立的渠道 / 模型 / 推理强度；Git 提交信息还可选简约或明细风格（缺省明细）。点「保存配置」写入 `$APPCONFIG/ai-settings.json`。开启提交信息后，Git 抽屉提交框右上角显示 Sparkles 按钮，调用 `generate_git_commit_message` 覆盖 textarea，不自动提交。开启会话标题后，新建会话在 INSERT 截断标题之后异步 one-shot 生成标题，经 `native-session-title` 刷新侧栏；续聊 / 恢复 / 分叉不重生成。使用统计顶部提供 7 天 / 30 天 / 自定义筛选，概览展示总 Token（k/M）与缓存率，并有活跃热力图、按天 Token 趋势、模型用量三张卡，数据来自 `get_native_usage_analytics`。开关即时生效，文本输入配「保存」。通用设置提供桌面通知开关；Native runtime 提供工具后自动 checkpoint 开关和保留天数（`0` 不清理）；SSH 新建/编辑弹窗页脚可先保存再测试连接或探测密码认证；弹窗提供 KEX / Host Key / Cipher / MAC 高级算法区，以及旧服务器预设 / 恢复默认；MCP 卡片可在全部工作区和指定工作区之间切换并勾选绑定工作区。
