...
点击文件或会话内的改动文件入口打开 `GitDiffDialog`，不再在列表底部嵌入小预览。弹窗宽 92vw（最大 1440px）、高 85dvh，距离窗口边缘至少 16px；展示路径和工作区 / 暂存范围，复用 `DiffView` 的高亮、双行号与换行设置。包含加载、空差异、错误重试、二进制及截断状态；关闭、换文件或换工作区后忽略旧请求。预览请求消费后清空，同一文件可反复打开。
