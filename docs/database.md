...
受控 `Write` / `Edit` / `ApplyPatch` 的路径级快照，不把整库检查点当成某一条消息的改动。列包括 `session_record_id`、`call_id`、`tool_name`、`path`、`execution_target`、`target_root`、`change_kind`（`added` / `modified` / `deleted` / `unsupported`）、`before_sha` / `after_sha`、`before_text` / `after_text`、`created_at`。子 Agent 的写入记在父工具调用的 `call_id` 上。

### `native_file_rollbacks`

一次文件回滚操作：`request_id` 唯一，`status` 为 `applying` / `compensating` / `completed` / `failed` / `needs_recovery`，`backup_json` 在写入前落库。`needs_recovery` 表示补偿时碰到更新的修改，备份保留且会话不能继续。
...
- 应用配置目录里的 `mcp-servers.json`、`native-settings.json`（含钩子）、`network-settings.json`、`quick-prompts.json`、`ai-settings.json`
...
应看到 26 张业务表加 `_sqlx_migrations`，且 version 19 成功。
