...
关闭主窗口后隐藏到托盘，不退出进程，也不写入窗口尺寸或位置。启动时使用 `tauri.conf.json` 的默认尺寸（1280×800），显示前后都屏幕居中。本次进程内仍可拖动和缩放；托盘左键、菜单「显示窗口」或 macOS 点 Dock 图标（`RunEvent::Reopen`）再次显示时重新居中，尺寸保持到进程退出。
...
更新重启总清理预算 5 秒：会话持久化与 MCP 收尾最多 3 秒，SSH 与数据库各最多 1 秒，提前完成即继续。普通退出总预算 30 秒，保留空闲会话的记忆收尾机会；快速重启跳过记忆提取。退出期间停止自动化、新会话、追加输入与新 SSH 连接，所有会话同时收到结束信号，超时取消并中止剩余任务，不无限等待 join。SQL 插件在清理开始时注销，其已创建的连接池保留到最后限时关闭，避免插件的 `Exit` 钩子重复阻塞。预算不包含操作系统启动新进程的耗时；超时前未持久化的内存内容可能无法完整保存。
