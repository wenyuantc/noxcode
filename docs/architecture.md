...
3. `setup` 创建托盘（主窗口先隐藏）。
4. debug 下异步打印迁移状态。
5. `RunEvent::Ready` 时显示主窗口。尺寸用 `tauri.conf.json` 的默认 `1280×800`，显示前后都调用 `center()` 屏幕居中，不读取 `$APPCONFIG/window-state.json`。若预检失败，弹中文错误对话框后 `exit(1)`。预检必须等事件循环就绪再弹窗，不能在 `setup` 里阻塞主线程。
6. 关闭主窗口后隐藏到托盘，不保存尺寸或位置。本次进程内仍可拖动和缩放；从托盘或 macOS Dock（`RunEvent::Reopen`）再次显示时重新居中，尺寸保持到进程退出。`RunEvent::Exit` 取消 Agent 并关闭 SshPool。
