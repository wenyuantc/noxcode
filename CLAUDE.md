...
**noxcode** is a Tauri v2 desktop app: an in-process native Agent with AI channels, SSH workspaces, and Git checkpoints. See `plan.md` for the full build plan.
...
- SQLite lives at `$APPCONFIG/noxcode.db`. Migration versions in `db/migrations.rs` must stay contiguous `1..N`; this will be enforced by `migration_versions_are_contiguous` (P1).
- The only runtime external dependency is system `git` ≥ 2.11. Startup preflight fails hard below that.
