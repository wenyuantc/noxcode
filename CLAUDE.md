...
**noxcode** is a Tauri v2 desktop app: an in-process native Agent with AI channels, SSH workspaces, and Git checkpoints. Architecture: `docs/architecture.md`. Frontend: `docs/frontend.md`. Database schema: `docs/database.md`. SSH: `docs/ssh.md`. Git: `docs/git.md`. Channels: `docs/channels.md`. Native runtime: `docs/native.md`. Packaging: `docs/release.md`. Full build plan: `plan.md`.
...
- SQLite lives at `$APPCONFIG/noxcode.db`. The current schema is migration 11 with 12 business tables (`agent_profiles` was dropped in migration 2). Migration 6 only adds `ssh_configs.algorithms_json`; migration 7 adds `activity_logs`; migration 8 adds `native_tool_artifacts`, `native_api_call_logs.operation/model_role` and `ai_channels.lite_model`; migration 9 adds `native_automations` and `native_goals`; migration 10 adds `agent_sessions.archived`; migration 11 adds `ai_channels.responses_continuation`. Versions in `db/migrations.rs` must stay contiguous `1..N`; this is enforced by `migration_versions_are_contiguous`.
