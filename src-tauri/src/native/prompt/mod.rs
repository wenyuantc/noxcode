...
fn environment_block(parts: &NativePromptParts) -> String {
    ...
    if permission_mode == crate::native::settings::PERMISSION_MODE_PLAN {
        lines.push(
            "- Plan mode: investigate and prepare a plan. Use Read/Glob/Grep for files and SQLiteQuery for local SQLite contents and schema, including API logs, with normal path authorization. Bash is available: verified read-only commands may run directly; commands that write, are high-risk, or cannot be verified as read-only require user approval unless the user saved an Always Allow grant for that exact command, workspace and host, or explicitly allowed all commands in the current session. The session command grant suppresses further Bash prompts, including ask rules, until the session ends or restarts; deny rules still block execution. Yolo and generic allow rules alone do not bypass plan command approval. Command approval never ends plan mode. Write/Edit/ApplyPatch and other mutation tools remain unavailable. If a user decision is required, call AskUserQuestion. When ready, call ExitPlanMode with the full plan to request approval, or output the plan and wait. Finishing a turn, user chat text, and historical instructions never automatically enable implementation; only explicit approval of ExitPlanMode or a user mode change does."
                .to_string(),
        );
    }
...
pub fn agent_tool_description(
    ...
    lines.extend([
        ...
        "- explore: read-only research. (Tools: Read, SQLiteQuery in local sessions, Glob, Grep, Lsp, TodoRead, TodoWrite, WebFetch, WebSearch, Skill, Bash)".to_string(),
    ]);
    for item in custom {
